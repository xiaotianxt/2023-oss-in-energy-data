#!/usr/bin/env python3
"""
Create Detailed Scorecard CSV

This script creates a CSV file with repository names, URLs, and detailed scores
for all individual Scorecard security checks, plus criticality scores.
"""
        ...
        if 'Repository:' in section and 'Status: SUCCESS' in section:
            try:
                ...
                for check_name in scorecard_checks:
                    # Look for the score line for this check
...
def create_detailed_csv():
    ...
    print("=" * 70)
    print("CREATING DETAILED SCORECARD CSV WITH ALL SECURITY CHECKS")
    print("=" * 70)
    ...
    output_filename = 'repository_detailed_scores.csv'
    ...
    # Show score ranges
    scorecard_scores = final_df['scorecard_aggregate'].dropna()
    criticality_scores = final_df['criticality_score'].dropna()
    
    if len(scorecard_scores) > 0:
        print(f"\nScorecard aggregate scores: {scorecard_scores.min():.2f} - {scorecard_scores.max():.2f} (avg: {scorecard_scores.mean():.2f})")
    
    if len(criticality_scores) > 0:
        print(f"Criticality scores: {criticality_scores.min():.4f} - {criticality_scores.max():.4f} (avg: {criticality_scores.mean():.4f})")
    
    # Show individual check statistics
    print(f"\nIndividual Scorecard Check Coverage:")
    for col in scorecard_columns:
        if col in final_df.columns:
            non_null_count = final_df[col].notna().sum()
            if non_null_count > 0:
                avg_score = final_df[col].mean()
                check_name = col.replace('scorecard_', '').replace('_', '-').title()
                print(f"  {check_name:<25}: {non_null_count:3d} repos (avg: {avg_score:.1f})")
    
    # Show a sample of the data
    print(f"\nSample data (first 3 repositories):")
    sample_cols = ['repository_name', 'scorecard_aggregate', 'criticality_score'] + scorecard_columns[:5]
    existing_sample_cols = [col for col in sample_cols if col in final_df.columns]
    print(final_df[existing_sample_cols].head(3).to_string(index=False))
    
    print(f"\n✅ Detailed CSV created successfully!")
    print(f"📁 File: {output_filename}")
    print(f"📊 Contains all {len([col for col in scorecard_columns if col in final_df.columns])} individual Scorecard security checks")
